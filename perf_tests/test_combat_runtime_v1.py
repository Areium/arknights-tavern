...
class SaveRoundTripTests(unittest.TestCase):
    """P0-5：to_dict/from_dict 完整往返 + v0→v1 迁移钳制。"""

    def _populated(self):
        random.seed(53)
        a = make_player("A", hp=300)
        b = make_player("B", hp=300)
        e = make_enemy("E1", max_ap=3, action_slots=2)
        e2 = make_enemy("E2", max_ap=3, action_slots=1)
        eng = build_engine([a, b], slash_cards("A", 4) + slash_cards("B", 4),
                           [(e, (0, 1)), (e2, (1, 1))])
        eng.load_waves([])  # noop
        eng.start_battle()
        card = next(c for c in eng.shared_pool.hand if c.cost == 1)
        eng.play_card("A", card, (0, 1))
        eng.end_player_round()
        return eng
    ...
    def test_full_round_trip(self):
        eng = self._populated()
        data = eng.to_dict()
        self.assertEqual(data["balance_version"], 1)
        restored = CombatEngine.from_dict(data)

        self.assertEqual(restored.balance_version, 1)
        self.assertEqual(set(restored.units), set(eng.units))
        for uid, u in eng.units.items():
            r = restored.units[uid]
            self.assertEqual(r.hp, u.hp)
            self.assertEqual(r.AP, u.AP)
            self.assertEqual(r.HEAL, u.HEAL)
            self.assertEqual(r.attributes, u.attributes)
            self.assertEqual(r.status, u.status)
            self.assertEqual(r.action_slots, u.action_slots)
            self.assertEqual(r.power_tier, u.power_tier)
            self.assertEqual(r.role, u.role)
            self.assertEqual(r.threat_points, u.threat_points)
        self.assertEqual(restored.shared_ap, eng.shared_ap)
        self.assertEqual(restored.SHARED_AP_MAX, eng.SHARED_AP_MAX)
        self.assertEqual(restored.state.round_num, eng.state.round_num)
        self.assertEqual(restored.state.phase, eng.state.phase)
        self.assertEqual(restored.wave_num, eng.wave_num)
        self.assertEqual(restored.telemetry, eng.telemetry)
        self.assertEqual(restored.enemy_actions_taken, eng.enemy_actions_taken)
        self.assertEqual(restored.initial_hp, eng.initial_hp)
        # 牌堆逐张等价
        for uid, pool in eng.enemy_pools.items():
            self.assertEqual([c.card_id for c in pool.deck],
                             [c.card_id for c in restored.enemy_pools[uid].deck])
        self.assertEqual([c.card_id for c in eng.shared_pool.hand],
                         [c.card_id for c in restored.shared_pool.hand])
        self.assertEqual([c.card_id for c in eng.shared_pool.exhaust],
                         [c.card_id for c in restored.shared_pool.exhaust])

    def test_pending_waves_round_trip(self):
        random.seed(59)
        a = make_player("A", hp=300)
        eng = build_engine([a], slash_cards("A", 6), [])
        w1 = make_enemy("W1", max_ap=3)
        w2 = make_enemy("W2", max_ap=3, action_slots=2)
        eng.load_waves([[(w1, (0, 1))], [(w2, (1, 1))]])
        eng.start_battle()
        self.assertEqual(eng.wave_num, 1)
        data = eng.to_dict()
        restored = CombatEngine.from_dict(data)
        self.assertEqual(restored.wave_num, 1)
        self.assertEqual(len(restored.pending_waves), 1)
        self.assertEqual(restored.pending_waves[0][0][0].name, "W2")
        # 恢复后继续可战斗：清完第一波 → 第二波入场
        eng.units["W1"].hp = 0
        eng.grid.remove_unit(eng.units["W1"])
        self.assertFalse(eng._check_battle_end())
        self.assertEqual(eng.wave_num, 2)
        self.assertIn("W2", eng.units)

    def test_v0_migration_clamps_ap(self):
        data = {
            "balance_version": 0,  # 旧存档
            "shared_ap": 99,
            "shared_ap_max": 7,
            "units": {
                "A": make_player("A").to_dict() | {"AP": 99, "MAX_AP": 2},
            },
            "shared_pool": None,
            "enemy_pools": {},
            "pending_waves": [],
            "round_num": 3, "phase": "PLAYER_TURN", "winner": "",
        }
        eng = CombatEngine.from_dict(data)
        self.assertEqual(eng.SHARED_AP_MAX, 4, "v1 规则：战术规划 5 → 上限 4")
        self.assertEqual(eng.shared_ap, 4, "旧存量 99 应钳制到新上限，不凭空增加")
        self.assertEqual(eng.units["A"].AP, 2, "个人 AP 钳制到 MAX_AP")

    def test_v0_migration_tp8(self):
        data = {
            "shared_ap": 99,
            "units": {"A": make_player("A").to_dict() |
                      {"attributes": {**BASE_ATTRS, "tactical_planning": 8}}},
            "shared_pool": None, "enemy_pools": {}, "pending_waves": [],
            "round_num": 1, "phase": "ROUND_START", "winner": "",
        }
        eng = CombatEngine.from_dict(data)
        self.assertEqual(eng.SHARED_AP_MAX, 5)
        self.assertEqual(eng.shared_ap, 5)
