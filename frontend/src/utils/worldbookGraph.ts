...
export function buildWorldBookGraph(detail: WorldBookDetail, policy: WorldBookPolicyDraft, options: {
  ...
  const priority = (uid: string) => uid === options.focusedUid ? 0 : configured.has(uid) ? 1 : 2;
  const entries = matches.length > limit
    ? [...matches].sort((a, b) => priority(a.uid) - priority(b.uid)).slice(0, limit)
    : matches;
  ...
  for (const category of categories) {
    if ((!branch || branch.has(category.id)) && !options.connectedOnly &&
  ...
  const nodes: WorldBookGraphNode[] = flattenCategoryTree(categories)
  ...
  if (options.view === "dependencies") {
    for (const edge of policy.dependency_edges) {
      const from = entryNodeId(edge.from_uid), to = entryNodeId(edge.to_uid);
      if (ids.has(from) && ids.has(to)) edges.push({ id: dependencyEdgeId(edge.from_uid, edge.to_uid), from, to, kind: "dependency" });
    }
  }
  return { nodes, edges, entryCount: entries.length, hiddenCount: matches.length - entries.length };
}
