...
- **选项填充到输入框** — 控制点击选项后的行为
- **叙述思考档位** — 控制剧情叙述/角色对话的 LLM 思考强度（关闭/低/中/高，默认关闭最快）。
  混合思考模型（如 DeepSeek V4）即使"关闭思考"缺省也会自动思考；本选项显式发送
  `reasoning_effort=none` 才能真正关闭。选项/回忆等分类任务始终使用关闭档位。

### 文档编辑

主页主菜单或管理页顶栏进入「🗂️ 内容中心」统一管理全部内容：**角色·剧情** Tab 浏览和编辑数据文档（角色、物品、世界观设定等，随程序分发的方舟内容开箱即用），支持「⬆角色卡」一键导入 SillyTavern 角色卡（角色设定 + 内嵌世界书）；**索引** Tab 管理文档间 `imports` 引用关系与会话白名单；**资产 / 卡牌** Tab 管理图片与战斗卡牌。

### 世界书

内容中心「📖 世界书」Tab 管理酒馆（SillyTavern）Lorebook 兼容的世界书（预装整合包与用户导入内容在同一列表统一管理）：

- **导入** — 支持酒馆世界书导出 JSON（v1/v2）、角色卡内嵌世界书、聊天备份 `.jsonl`，自动探测格式并报告导入/跳过/警告
- **触发机制** — 条目按主/副关键词扫描最近对话自动注入叙述与对话的提示词；支持常驻、选择性、概率、大小写、全词匹配等酒馆语义
- **绑定** — 每本书可绑定到指定会话，未绑定时回落到全局默认书；均未配置时使用预装整合包（方舟设定集）
- **来源与重装** — 预装包（青标）随程序分发、首次启动自动安装，可编辑/停用/删除/一键重装；导入内容（紫标）自由扩展
