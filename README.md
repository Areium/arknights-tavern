...
- **场景角色** — 浏览角色库，加载/卸载角色到场景。点击角色名切换对话目标，悬停查看详细属性/关系/背景，支持 `[@角色名]` 语法在对话中切换。可固定详情卡片并在线编辑角色数据，修改仅影响当前会话，不改变模板。
...
- **多角色同场** — 多个角色可同时在场，共享场景上下文和事件日志。`[@角色名]` 语法切换对话目标，不在场景中的角色会触发提示。
