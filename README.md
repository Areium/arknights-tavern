...
![对话界面：剧情叙述、行动选项与回退按钮](docs/images/03-chat.jpg)

> ⚠️ 此图尚未提供，当前为占位。补图要求见文末「附图清单」。
...
![剧情节点图：整页画布编辑剧情节拍与战斗节点](docs/images/06-plot-graph.jpg)
...
**Q1：界面提示「后端未连接」，或者对话一直没有回复？**
先确认启动后端和前端的那两个窗口（或一键脚本窗口）都还开着。然后在「设置 → LLM 配置」点「测试连接」验证模型可用性；若配置刚改过，点「刷新 LLM 列表」让后端重新探测。

**Q2：端口 5000 或 5173 被占用，启动失败？**
推荐直接用一键脚本，它会结束**本项目**遗留的 Python / Node / Electron 进程后再启动，不会误杀其他程序。若确实需要更换端口：后端可用环境变量 `API_HOST` / `API_PORT`，但前端 Vite 的 `/api` 代理目标写死在配置里，必须同步修改 `frontend/vite.config.ts`（或 `vite.config.web.ts`）中的代理地址，否则前端会连不上后端。

**Q3：我的存档在哪里？怎么备份 / 换电脑？**
所有游戏数据都在仓库根目录的 `data/` 文件夹里：会话记录在 `data/memory/`，世界书在 `data/worldbooks/`，角色与物品在 `data/characters` / `data/items`，战斗数据在 `data/combat/`。直接拷贝整个 `data/` 目录即可完成备份或迁移。也可以只导出单个会话：会话大厅里点「💾 导出存档」得到 `.zip`，在另一台机器上「📂 导入存档」即可。注意 `config/llm_config.json` 保存着你的 API Key，分享前请先删除。

**Q4：必须联网吗？能不能完全离线运行？**
除调用 LLM 之外，剧情推进、世界书注入、战斗与记忆检索都在本地完成；向量记忆使用本地 ONNX 嵌入（不依赖云端 embedding 服务），并且关闭了匿名遥测。配合本地 Ollama 模型即可完全离线游玩。唯一例外是界面字体从 Google Fonts 加载，无网络时会回退到系统字体——不影响功能。

**Q5：我从酒馆（SillyTavern）过来，原来的角色卡和世界书能直接用吗？**
可以。世界书支持导入酒馆导出的 v1/v2 JSON、角色卡内嵌世界书以及 `.jsonl` 聊天备份，还能导出回酒馆 v1 格式；角色卡支持 `.png` / `.json` / `.webp` / `.jpg`，导入时会一并读取卡内嵌的世界书。

**Q6：世界书导入成功了，但对话里看不到效果？**
依次检查：① 这本书是否绑定到了当前会话（未绑定时回落到全局默认书，都没配置则用预装整合包）；② 条目是否处于启用状态；③ 触发关键词是否出现在最近若干轮对话里（受条目的扫描深度限制）；④ 条款是否设置了概率、选择性或分组权重，可能被跳过。可在「内容中心 → 世界书图谱」用「分类结构 / 条目依赖 / 依赖树」视图排查条目归属与依赖关系。

**Q7：为什么战斗页面不跟随我选的皮肤？**
这是有意设计：皮肤的作用域刻意排除了战斗页，战斗界面使用独立的固定配色以保证战场信息（地形、单位状态、可行动范围）在任意主题下都清晰可辨。
...
⚠️ **本仓库当前没有根级 `LICENSE` 文件。** 在分发、二次开发或商用之前，请先与项目作者确认授权方式。
...
```bash
bash scripts/run_tests.sh                    # 统一测试入口（pytest + tests/legacy/）
python tools/validate_battle_spec.py         # 校验战斗节点规格
python tools/simulate_battle.py              # 试跑战斗
python scripts/generate_builtin_worldbook.py # 重新生成预装世界书整合包
python tools/generate_combat_backgrounds.py  # 批量生成战斗背景图
```
...
存放目录：docs/images/          命名规范：NN-<英文短名>.jpg（两位序号 + 小写短横线）
推荐规格：JPEG，宽度 1600–1920px，深色主题下截取，不要包含个人 API Key / 会话内容

已有（无需处理）：
  - 01-main.jpg        主界面（主菜单 + 状态栏）
  - 02-sessions.jpg    会话大厅
  - 04-documents.jpg   内容中心 · 资产 / 卡牌
  - 05-worldbook.jpg   世界书条目管理
  - 07-combat.jpg      战斗界面
  - 08-settings.jpg    设置页面

待补（README 中已用占位图引用，补图后占位提示需删除）：
  - 03-chat.jpg        对话界面：需要同时体现
                        ① 剧情叙述正文 + 行动选项
                        ② 轮次之间的「↩ 回退」按钮
                        ③ 叙述右下角的 `◂ 1/1 ▸` 版本切换器
                        ④ 左侧面板底部的「回忆」卡片
                        （自由模式的场景角色 / 环境面板可作为第二张补充）
  - 06-plot-graph.jpg  剧情节点图：整页画布上的节点连线视图，
                        最好能同时看到节点编辑抽屉（剧情节拍或战斗节点表单）

可选补充（若能让 README 更完整）：
  - 09-create-session.jpg  新建会话向导（模式选择 + 战斗模式选择 + 世界书绑定）
  - 10-replay.jpg          叙述变体重新生成 / 回退确认对话框的实际操作流程

补完图后，请把「界面速览」小节中两处 `> ⚠️ 此图尚未提供，当前为占位…` 的引用块一并删除。
═══════════════════════════════════════════════════════════════════════
-->
