...
明日方舟主题文字 RPG：剧情模式（LLM 驱动叙事 + 选项 + 记忆 + 环境）、自由模式（沙盒角色交互）、7×7 等距网格回合制战斗（CSS 3D 网格 + PixiJS Spine 骨骼动画覆盖层）。
...
### 核心后端（`src/`）
- `app.py` — Flask factory `create_app()`，注册所有 Blueprint，入口 `main()`
...
- `session_overlay.py` — 职责聚合：角色/物品属性覆盖 + 剧情日志（保留最近 15 条）+ 节拍状态 + 任务系统
...
API 层（`src/blueprints/`）：Flask Blueprint — `chat.py`（对话/叙述/SSE/战斗触发）、`combat.py`（战斗 SSE）、`cards.py`（卡牌 JSON CRUD）、`documents.py`、`sessions.py`、`scene.py`、`environment.py`、`index.py`、`wiki.py`、`llm.py`、`assets.py`、`memories.py`、`status.py`。

战斗引擎（`src/combat_engine/`）：`engine.py`（回合循环/AP/士气）、`entity.py`（CombatUnit）、`grid.py`（7×7 寻路/范围）、`card.py`（卡牌/CardPool）、`card_data.py`（职业基础卡牌）、`card_loader.py`（JSON → 卡牌实例）、`dice.py`。

服务层（`src/services/`）：`buff_pool.py`、`dice.py`。共享工具（`src/shared/`）：`helpers.py`（SSE 响应工厂、记忆注入）、`cache.py`。Provider（`src/providers/`）：`openai.py`、`deepseek.py`。

### 前端架构

入口 `main.tsx`（React 18 createRoot）→ `App.tsx`：Sidebar（导航）+ StatusBar（状态栏）+ 内容区（ChatView / CombatView / DocumentManager / IndexManager / SettingsPanel）。轮询后端状态 5s、LLM 状态 10s、会话列表 15s。
...
- `components/combat/` — CSS 网格（CombatGrid）+ PixiJS Spine 覆盖层（PixiCombatScene，runtime-3.8）+ 手牌（CombatHand）+ 卡组查看（DeckViewer）+ 卡牌编辑（CardEditor）+ 状态/事件面板
...
工具：`utils/dialogueParser.ts` — 解析 `「」` 对话为 `DialogueSegment[]`，前文叙述匹配场景角色名确定说话人。`components/MarkdownRenderer.tsx` — 统一 Markdown 渲染。
