...
- 生成使用 wb-dep-v2 提示词。正文逐段完整覆盖；长行切开、短章节合并，每片不超过 1800 字符。分块卡片先持久化，全部成功才合并缓存。默认不丢弃正文或明确引用。通用触发词按文档频率过滤，名称/UID 豁免，报告记录过滤词和频率。
- 当前安装副本 261 条 / 727945 字符：704 分块、1041 候选、估算 249 次；全书确定性 stub 实跑 249 次完成。每次启动/继续最多 400 次 chat（含 JSON 修复），不自动提升为数千次。真实模型可能返回更多候选或需要修复，超限暂停并保留断点，用户显式继续。
