...
| UI/动作/音效/背景 | ✅ 3D 网格 + Spine 动画 + 音效 + 战斗背景 + 伤害数字/粒子 + 敌人意图头顶图标 + 闪避/未命中浮动文字 + 伤害定位修复 + 状态效果音效 | 状态效果专属 Spine 动作可后续精化 |
...
### P2 · 战斗深度 + 难度曲线 + 卡组
4. 状态效果运行时：✅ 护盾/减速/束缚/虚弱/增幅 + 沉默/燃烧DoT 已实装（feat/status-effects + feat/silence-burn：沉默阻止源石技艺卡牌、燃烧每回合掉血；supp_nullify/supp_disrupt/caster_burn 卡牌生效）；⏳ 嘲讽/闪避 待做。
