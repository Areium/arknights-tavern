...
## Codex 最终独立验证（2026-09-16）

实现由 WorkBuddy 完成，Codex 审核并复测。世界书相关回归共 255 项通过；最后正文哈希修正后的性能与缓存测试 30 项通过。前端构建通过。此前全量仓库测试存在战斗/LLM 环境、战斗数据和输出编码问题，不能据此宣称全仓库全绿。

真实调用使用主工作区 `config/llm_config.json`，经项目 `LLMBackendManager.get_llm()` 获取模型，未改写世界书或模型配置。相同六条真实条目（10,285 字符）记录如下：

| 运行 | 调用次数 | 实际输入 token | 实际输出 token | 实际总 token | 耗时 |
|---|---:|---:|---:|---:|---:|
| 旧实现基线 | 3 | 10,297 | 5,793 | 16,090 | 24.78 秒 |
| 新实现首次复测 | 2 | 9,022 | 4,663 | 13,685 | 20.20 秒 |
| 新实现最终验证 | 2 | 9,025 | 6,199 | 15,224 | 27.9 秒 |

两次新实现均保留了战斗规则到「战场机动」「战斗技巧」的两条必要依赖，证据来自原文。最终验证的缓存重跑为 **0 次模型调用**，候选覆盖 2/2、分析卡 6/6。

以上小样本的实际 token 减少约 5%–15%，响应时间存在波动，不能从单次结果宣称稳定的耗时降幅。262 条全书的 322→147 次请求、约 394 万→183 万输入 token 是基于真实书和历史分析卡的冷启动估算；全书真实冷启动耗时与关系质量仍未实测。原有历史全书任务确实使用了项目内的模型，其 19 个失败批次为响应缺失/无效等问题，而非未连接项目 LLM。
